<?php
/**
 * Contoh Script Bridge API Flickreels via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Flickreels di sistem Bridge
function callFlickreelsAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/flickreels/api/v1";

// Contoh 1: Fitur Cari Film Flickreels
echo "\n=====================[ 1. Pencarian Flickreels ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callFlickreelsAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callFlickreelsAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callFlickreelsAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/flickreels ◀◀◀\n\n";
